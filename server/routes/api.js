...
router.get('/list/:type/:id', async (req, res) => {
    ...
    try {
        ...
        const languages = snapshot.docs.map(doc => {
            const docId = doc.id;
            return docId.substring(queryPrefix.length);
        });
        ...
        res.status(500).json({ error: 'Failed to list subtitles from Firestore' });
    }
});
...
router.get('/subtitles/:type/:id/:lang', async (req, res) => {
    ...
    try {
        ...
        if (!docSnap.exists) {
            return res.status(404).send('Subtitle not found in Firestore');
        }

        const subtitleData = docSnap.data();
        if (!subtitleData || !subtitleData.content) {
            return res.status(500).send('Subtitle data is invalid in Firestore');
        }

        cacheManager.set(cacheKey, subtitleData.content);
        ...
        res.status(500).send('Failed to serve subtitle file from Firestore');
    }
});
...
router.post('/upload', upload.single('subtitleFile'), async (req, res) => {
    ...
    try {
        ...
        await docRef.set({
            type: contentType,
            imdbId: imdbId,
            ...(contentType === 'series' && { season: parseInt(season), episode: parseInt(episode) }),
            language: language,
            content: content,
        ...
        res.json({ message: 'Subtitle uploaded to Firestore successfully!' });

    } catch (err) {
        console.error('Error uploading subtitle to Firestore:', err);
        try { await fs.unlink(file.path); } catch (cleanupErr) { console.error('Error cleaning up temp file:', cleanupErr); }
        res.status(500).json({ error: 'Failed to save subtitle file to Firestore.' });
    }
});
...
router.get('/listall', async (req, res) => {
    ...
    try {
        const snapshot = await db.collection('subtitles').get();
        ...
        console.error('Error listing all subtitles from Firestore:', err);
        res.status(500).json({ error: 'Failed to list all subtitles from Firestore' });
    }
});
...
router.delete('/subtitles/:type/:id/:lang', async (req, res) => {
    ...
    try {
        ...
        if (!docSnap.exists) {
             return res.status(404).json({ error: 'Subtitle not found in Firestore, cannot delete.' });
        }
        ...
        res.json({ message: 'Subtitle deleted successfully from Firestore!' });

    } catch (err) {
        console.error(`Error deleting subtitle ${type} ${id} (${lang}) from Firestore:`, err);
        res.status(500).json({ error: 'Failed to delete subtitle file from Firestore.' });
    }
});
